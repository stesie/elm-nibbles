...
canvasWidth = boardWidth * dotDiameter
canvasHeight = boardHeight * dotDiameter
...
init : (Model, Cmd Msg)
init = (Model [] 3 Nothing North, randomizeGame)

randomPoint : Random.Generator (Int, Int)
randomPoint = Random.pair (Random.int 0 boardWidth) (Random.int 0 boardHeight)

randomizeGame : Cmd Msg
randomizeGame = Random.generate RandomizeGame (Random.pair randomPoint randomPoint)

-- UPDATE

type Msg = RandomizeGame (Point, Point) | Tick Time.Time | KeyDown Keyboard.KeyCode
...
keyCodeToDirection x = case x of
  37 -> Just West
  38 -> Just North
  39 -> Just East
  40 -> Just South
...
view : Model -> Html Msg
view model = div []
  [ renderSnake model.snake ++ renderFood model.food
      |> Collage.collage canvasWidth canvasHeight
      |> Element.toHtml
  ]

renderSnake : List Point -> List Collage.Form
renderSnake = List.map (renderPoint Color.green)

renderFood : Maybe Point -> List Collage.Form
renderFood x = case x of
  Nothing -> []
  Just p -> [renderPoint Color.red p]

renderPoint : Color.Color -> Point -> Collage.Form
renderPoint color (x, y) = Collage.circle 10
  |> Collage.filled color 
  |> Collage.move (toFloat x * dotDiameter - canvasWidth / 2 + dotDiameter / 2
